...
class Faucet extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      getCoins: false,
      auth_key_prefix: "",
      vCoin: [{ pathname: "/app/Violas", type: "vtoken" }],
      bCoin: [
  ...
  async get_auth_key_prefix(_chain) {
    ...
    await this.setState({
      auth_key_prefix: this.props.account_info.authentication_key.slice(0, 32),
    });
  }
  ...
  async handleSubmit() {
    ...
    if (!match_published) {
      await this.props.getWarning(
        `This address are not published ${this.state.coinId.show_name} from ${this.state.coinId.chain}`
      );
      return;
    }
    if (this.getAgainClick()) {
      await this.get_auth_key_prefix(this.state.coinId.chain);
      await this.props.getCoinsFun(
        this.state.coinAddress,
        this.state.coinId.name,
        this.state.auth_key_prefix,
        this.state.coinId.chain
      );
    }
  }
  //判断是否重复点击
  getAgainClick() {
    ...
    if (currentClickTime - preClickTime < 3000) {
      //如果是3秒内重复点击
      this.props.getWarning(
        "Please do not get the currency of this currency repeatedly"
      );
        ...
        <div className="container1">
          <div className="container2">
            ...
            <div className="inLine1">
              <h4>Address:</h4>
              <textarea
                className={this.state.getFocus ? "input focusAct" : "input"}
                onFocus={() => {
                  this.setState({
                    getFocus: true,
                  });
                ...
                defaultValue={this.state.coinAddress}
              ></textarea>
                ...
                >
                  ...
                  <span>
                    {this.state.showDropDown ? (
                      ...
                      <img src="/img/xiala@2x (1).png" alt="" />
                    )}
                  </span>
                </div>
                {this.state.showDropDown ? (
                      ...
                      {this.state.MarketCurrencies.length > 0 ? (
                        this.state.MarketCurrencies.map((item, ind) => {
                          return (
                            <li
                              key={ind}
                              onClick={() => this.handleChange("id", item)}
                            >
                              {item.show_name} ({item.chain})
                            </li>
                          );
                        })
                      ) : (
            ...
            <div className="submit">
              <button ref="btn" onClick={this.handleSubmit.bind(this)}>
                Submit
              </button>
