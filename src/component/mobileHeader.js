  ...
  render() {
    ...
    return (
        <div className="mobileHead">
            ...
            {
                this.state.showRouters ? <div className="mark">
              <div className="headList">
               <div className="leftList">
                   <span onClick={() => this.showMenu('violas',0)} className={showMenuId == 0 ? 'active' : null}>Violas</span>
                   <span onClick={() => this.showMenu('BTC',1)} className={showMenuId == 1 ? 'active' : null}>BTC</span>
                   <span onClick={() => {
                       this.setState({
                        showRouters:false
                       },()=>{
                        this.props.history.push('/app/Libra')
                       })
                   }}>Libra</span>
               </div>
               <div className="rightList">
                   {this.state.showMenuId == 0 ? (this.state.vCoin.map((v, i) => {
                    return <NavLink onClick={()=>{
                        this.setState({
                            showRouters:false
                        })
                    }} to={v.pathname} activeClassName='active' key={i}>{v.type}</NavLink>
                    })) : (null)
                    }
                   {this.state.showMenuId == 1 ? (this.state.bCoin.map((v, i) => {
                    return <NavLink onClick={()=>{
                        this.setState({
                            showRouters:false
                        })
